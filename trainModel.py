...
processed_train_dir = 'processed_train_segmented'
...
def preprocess_and_save_images(source_dir, target_dir):
    """
    Preprocess images by applying hand segmentation and saving to target directory
    """
    ...
    for class_name in os.listdir(source_dir):
        ...
        for img_name in os.listdir(class_dir):
            ...
            # Segment hand
            segmented_hand, _, _ = hand_segmenter.segment_hand(img)
            
            if segmented_hand is not None:
                # Save segmented image
                target_path = os.path.join(target_class_dir, img_name)
                cv2.imwrite(target_path, segmented_hand)
            else:
                print(f"Warning: Could not segment hand in {img_path}")

# Preprocess training data
print("Preprocessing training data...")
preprocess_and_save_images(train_dir, processed_train_dir)
...
train_ds = train_datagen.flow_from_directory(
    processed_train_dir,  # Use segmented data
